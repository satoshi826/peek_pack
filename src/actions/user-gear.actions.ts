...
const userGearRepo = new UserGearFileRepository(cameraMasterRepo, lensMasterRepo, makerRepo);
...
export async function getUserOwnedGears(userId: string): Promise<UserGearWithDetails[]> {
  return userGearRepo.findByUserIdAndStatusWithDetails(userId, "owned");
}
...
export async function getUserWantedGears(userId: string): Promise<UserGearWithDetails[]> {
  return userGearRepo.findByUserIdAndStatusWithDetails(userId, "wanted");
}
